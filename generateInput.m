function [simulation]=generateInput
...
success=0;
while success==0
    prompt='1) Enter the number feast and famine cycles you want to simulate: (key in a number) > (example=1000)';
    temp=input(prompt);
...
success=0;
while success==0
    prompt='3) Enter the number of antibiotics that can be present in the sytem: (key in a number, 1 or 2) > (example=1)';
    temp=input(prompt);
    if (temp==[1])||(temp==[2])
...
success=0;
while success==0
    prompt='4) Enter the number different phenotype you would like to key in: (key in a number) > (example=1)';
    nump=input(prompt);
    
    if (rem(nump,1)==0)&&nump>0
       success=1;
    else
         fprintf('error\n');
    end
...
simulation.Phenotypes{ts}=[];
for i=1:nump
   success=0;
   while success==0
        prompt=['5) Enter the phenoypes you want to put in for species #',num2str(i),' :\n',...
           ' (key in an array of length that is equal to the answer of the third \n',...
           ' question, where each entry is a number, [-Inf,-1) for degrader phenotype \n',...
           ', [-1] for sensetive, [1] resistant, (1,Inf] for antibiotic producer) (for \n',...
           'example for two antibiotics, ''[100,100]'' is a strain that produces both \n',...
           ' antibiotics, ''[1,-100]'' is a strain that is resistant to first antibiotic \n',...
           ' and degrader of the second antibiotic) > (example=[-1,-1])'];
        temp=input(prompt)';
        
        if length(temp)==simulation.NumAnti

           simulation.Phenotypes{ts} =[   simulation.Phenotypes{ts},temp];
           success=1;
        else
            fprintf('error\n')
        end
   ...
   success=0;
   while success==0
        prompt='6) Enter the abundance of the phenotype #1: > (default=[1])';
        temp=input(prompt);

        if (temp<=1)&&(temp>=0)
            simulation.X{ts}(i,1)=temp;
            success=1;
        else
            fprintf('error\n');
        end
   ...
   if sum(simulation.X{ts})~=1
      fprintf('abundances of phenotypes do not add up to one, rescaling...') 
      simulation.X=simulation.X./sum(simulation.X);
